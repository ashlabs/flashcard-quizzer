...
class TestFlashcard:
    ...
    def test_flashcard_stores_front_and_back_text(self):
    ...
    def test_is_correct_accepts_case_insensitive_answer(self):
        """Test that an answer differing only in case is accepted."""
        card = Flashcard("la biblioteca", "the library")
        assert card.is_correct("The Library")

    def test_is_correct_ignores_surrounding_whitespace(self):
        """Test that leading/trailing whitespace in an answer is ignored."""
        card = Flashcard("la biblioteca", "the library")
        assert card.is_correct("   the library   ")

    def test_is_correct_rejects_incorrect_answer(self):
